 ...
 *
 * A primitive compile-time Sudoku board solver, using backtracking across the
 * empty entries.
 *
 * The reason for this project was to play around with constexpr and demonstrate
 * to myself that it is indeed possible (although via this algorithm, highly
 * inefficient) to solve Sudoku boards at compile-time.
 */
...
isqrt(const T n) {
  ...
  // but there is no point, because we assume it exists and will be found before
  // that.
  for (T t{}; t < n; ++t)
    if (t * t == n)
      return t;
 ...
 * It should be initialized with an N * N array representing the contents, with
 * zeroes used to indicate spaces that need to be filled.
 *
 * Note: There is a Sudoku alias below that represents the typical 9 x 9 sudoku
 * board over unsigned integers.
 *
 * @tparam T the type of the board entries
 * @tparam N the dimension of the board
 */
template <typename T, size_t N> class GenSudoku {
public:
  ...
  constexpr unsigned operator()(size_t x, size_t y) const { return get(x, y); }
...
private:
  /// The contents of a section of the board: either a row, a column, or a
  /// quadrant.
  using BoardSection = std::array<T, N>;

  /// This is the square root of the dimension, i.e. the side length of the
  /// quadrants.
  ...
  constexpr bool rowValid(size_t x) const { return sectionValid(row(x)); }
  constexpr bool colValid(size_t y) const { return sectionValid(col(y)); }
  constexpr bool quadrantValid(size_t x, size_t y) const {
    return sectionValid(quadrant(x, y));
  }

  constexpr bool rowComplete(size_t x) const { return sectionComplete(row(x)); }
  constexpr bool colComplete(size_t y) const { return sectionComplete(col(y)); }
  constexpr bool quadrantComplete(size_t x, size_t y) const {
    return sectionComplete(quadrant(x, y));
  }

  /**
   * This was part of my original attempt to limit the backtracking to only
   * valid entries using bitsets. bitsets, however, are not usable at
   * compile-time as they are not constexpr.
   *
   * TODO: Represent a bitset as an integer type and do a similar calculation.
   * Then see the effect this has on compile-time.
   * I suspect it will be insignificant, as we throw away invalid boards right
   * away anyway.
   */
  //    constexpr Candidates coverage(const BoardSection &bs) const {
  //        Candidates c;
  //        std::for_each(std::cbegin(bs), std::cend(bs), [&c](const auto i) {
  //        c[i] = true; }); return c;
  //    }
  //
  //    // Determine the candidates, i.e. the bits set to 1, for pos (x,y).
  //    constexpr Candidates candidates(size_t x, size_t y) const {
  //        return (coverage(row(x)) | coverage(col(y)) | coverage(quadrant(x,
  //        y))).flip();
  //    }

  /**
   * Determine if a section of the board (could be a row, col, or quadrant) is
   * valid, i.e. does not violate constraints.
   */
  ...
  /**
   * Determine if a section of the board (could be a row, col, or quadrant) is
   * complete, i.e. does not contain any zeroes.
   */
...
public:
  ...
  constexpr bool isSolved() const { return isValid() && isComplete(); }

  /**
   * A very primitive Sudoku solver.
   * It simply uses backtracking across the board to try to complete it.
   * This is clearly not an efficient way to solve Sudoku boards as there are
   * many dead ends that could be pruned right away by using a more suitable
   * algorithm, such as Knuth's DLX.
   *
   * An example implementation of DLX used to solve Sudoku boards can be found
   * in my Python DLX GitHub repository:
   *
   * https://github.com/sraaphorst/dlxpy/blob/master/examples/sudoku.py
   *
   * The only reason for the choice of algorithm here is to explore compile-time
   * solving of Sudoku boards. It might be possible to implement a compile-time
   * implementation of a specific DLX instance, but it would be difficult and
   * require some bizarre array manipulation due to the fact that the entire
   * basis of DLX is through sophisticated pointer manipulation.
   *
   * @return a GenSudoku that, if the original board was solvable, should
